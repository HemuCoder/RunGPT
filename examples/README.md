...
1. **01_simple_chat.py** - 基础对话示例
   - 使用 SimpleAgent 进行简单对话
   - 演示基本的 Agent 创建和使用

2. **02_tool_calling.py** - 工具调用示例
   - 注册和使用工具
   - 演示工具参数验证和调用
...
4. **04_planner_executor.py** - 任务规划示例
   - 使用 PlannerAgent 分解任务
   - 使用 ExecutorAgent 执行任务

5. **05_memory_usage.py** - 记忆管理示例
   - 跨对话记忆管理
   - Thread 历史保存和加载

6. **06_custom_model.py** - 自定义模型示例
   - 创建自定义模型提供者
   - 注册和使用自定义模型

7. **07_streaming.py** - 流式输出示例
   - 启用流式输出
   - 实时显示生成内容
